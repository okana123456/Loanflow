...
create table if not exists public.bripta_sms_outbox (
  ...
  recipient text,
  message text,
  message_type text not null default 'repayment'
...
begin
  ...
  select new.business_id,new.id,l.id,c.id,c.full_name,'repayment'
  from public.loans l
...
-- Service-only delivery check. Test messages use the same wallet and outbox
-- audit as repayment SMS without creating a repayment or changing a loan.
...
begin
  ...
  select * into v_outbox from public.bripta_sms_outbox
  where business_id=p_business_id and client_name='System test'
  ...
  update public.bripta_sms_wallets set credits_used=credits_used+1,updated_at=now()
  ...
  ) values (p_business_id,'System test',v_phone,v_message,'test',1,1,'sending',1)
  returning * into v_outbox;

  return jsonb_build_object('ok',true,'outbox_id',v_outbox.id,'business_id',p_business_id,
    'recipient',v_phone,'message',v_message,'segments',1,'sender_id',v_wallet.sender_id);
end;
$$;
...
begin
  ...
  insert into public.bripta_sms_outbox(
    business_id,client_id,client_name,recipient,message,message_type,segments,status
  ) values (
    new.business_id,new.id,new.full_name,new.phone,v_message,'onboarding',v_segments,'queued'
  )
...
begin
  ...
  where client_id=p_client_id and message_type='onboarding'
  order by queued_at desc
  limit 1
  for update;

  if v_outbox.id is null then return jsonb_build_object('ok',false,'code','not_queued'); end if;
  if v_outbox.status='sent' then return jsonb_build_object('ok',true,'already_sent',true,'outbox_id',v_outbox.id); end if;
  if v_outbox.status in ('sending','delivery_unknown') then return jsonb_build_object('ok',false,'code',v_outbox.status,'outbox_id',v_outbox.id); end if;
  ...
  if v_wallet.credits_purchased-v_wallet.credits_used < greatest(1,v_outbox.segments) then
    update public.bripta_sms_outbox
      set status='blocked_no_credit',last_error='SMS credits are insufficient',updated_at=now()
    where id=v_outbox.id;
    return jsonb_build_object('ok',false,'code','insufficient_credits',
      'remaining',greatest(0,v_wallet.credits_purchased-v_wallet.credits_used));
  end if;
  ...
  update public.bripta_sms_wallets
    set credits_used=credits_used+greatest(1,v_outbox.segments),updated_at=now()
  where business_id=v_wallet.business_id;
  update public.bripta_sms_outbox
    set status='sending',credits_reserved=greatest(1,v_outbox.segments),
        attempts=attempts+1,last_error=null,updated_at=now()
  where id=v_outbox.id
  returning * into v_outbox;

  return jsonb_build_object('ok',true,'outbox_id',v_outbox.id,'business_id',v_outbox.business_id,
    'recipient',v_outbox.recipient,'message',v_outbox.message,
    'segments',greatest(1,v_outbox.segments),'sender_id',v_wallet.sender_id);
end;
$$;
...
revoke all on function public.bripta_claim_test_sms(text,text) from public,anon,authenticated;
...
grant execute on function public.bripta_claim_test_sms(text,text) to service_role;
